...
use game_state::winit::dpi::{LogicalPosition, LogicalSize};
...
mod vs {
    include! {concat!(env!("OUT_DIR"), "/assets/shaders/vs.glsl") }
}
mod fs {
    include! {concat!(env!("OUT_DIR"), "/assets/shaders/fs.glsl") }
}
...
impl VulkanoRenderer {
    fn create_swapchain(
        ...
        let caps = match surface.capabilities(physical.clone()) {
            ...
            Err(err) => {
                return Err(
                    format!("Unable to get capabilities from surface: {:?}", err).to_string(),
                )
            }
        };
        ...
        images
            ...
            .map(|image| {
                ...
                let fb = Framebuffer::with_dimensions(renderpass.clone(), dimensions)
                    .add(image.clone() as Arc<dyn ImageViewAccess + Send + Sync>)
                    .unwrap()
                    .add(depth_buffer.clone() as Arc<dyn ImageViewAccess + Send + Sync>)
                    .unwrap()
                    .build()
                    .unwrap();
    ...
    pub fn new(
        ...
        let surface: Arc<Surface<AMWin>> =
            unsafe { vulkano_win_patch::winit_to_surface(instance.clone(), window.clone())? };
        ...
        let framebuffers =
            Self::create_framebuffers(renderpass.clone(), images.clone(), depth_buffer.clone());
    ...
    pub fn upload_model(&mut self, model: Arc<game_state::model::Model>) {
        {
            ...
            let cmd_buffer = cmd_buffer_build
                .copy_buffer_to_image(item.diffuse_map.clone(), texture_init.clone())
            ...
            let after_future = prev
                .then_execute(self.queue.clone(), cmd_buffer)
                .expect(&format!(
                    "VulkanoRenderer(frame {}, upload_model() ) - unable to execute command buffer",
                    self.fps.count()
                ))
    ...
    fn render(&mut self, camera: &CameraFacet) {
        ...
        self.uniform_buffer
            .write()
            .map(|mut write_uniform| {
                write_uniform.proj = proj_mat.into();
            })
            .map_err(|err| println!("Error writing to uniform buffer {:?}", err));
