...
mod tests {
    ...
    #[test]
    fn traverse_nodes() {
        let root_ptr = Node::create(0, 0, None);
        let child_ptr = Node::create(42, 0, Some(root_ptr.clone()));
    ...
    #[test]
    fn find_root() {
        let root = Node::create(0, 0, None);
        let child = Node::create(1, 0, Some(root.clone()));
    ...
    fn is_child_of_reparent() {
        let root = Node::create(0, 0, None);
        let im = Node::create(1, 0, Some(root.clone()));
        let child = Node::create(2, 0, Some(im.clone()));
        assert!( Node::is_child_of(child.clone(), im.clone()) );
        assert!( Node::is_child_of(child.clone(), root.clone()) );

        let root2 = Node::create(3, 0, None);
    ...
    fn fails_to_reparent_causing_a_cycle() {
        let root = Node::create(0, 0, None);
        let child = Node::create(1, 0, Some(root.clone()));
        let sibling = Node::create(2, 0, Some(root.clone()));
