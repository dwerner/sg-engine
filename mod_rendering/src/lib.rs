...
fn create_tentacle(x:f32, y:f32, z:f32, count:u32) -> Arc<SceneGraph> {
    ...
    let mx = Matrix4::from_translation(Vector3::new(0.0, 0.5, -0.5)) * Matrix4::from_scale(1.0);
    let mut child = Node::create( Box::new(Model::create("otherfile",mx)), Some(root.clone()) );
    for i in 0..count {
        let mx = Matrix4::from_translation(Vector3::new(0.5, 0.5, -0.5)) * Matrix4::from_scale(1.0);
